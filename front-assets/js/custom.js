(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(".all-patner").owlCarousel({
            ...
            items: 4,
            navText: ["<i class='fa fa-angle-left'></i>", "<i class='fa fa-angle-right'></i>"],
            responsive: { 0: { items: 2 }, 600: { items: 2 }, 768: { items: 2 }, 992: { items: 4 }, 1000: { items: 4 } },
        });
        $(".gallery-photo").magnificPopup({ type: "image", gallery: { enabled: true } });
        ...
        $(".project-gallery").imagesLoaded(function () {
            $(".filtr-container").isotope({ itemSelector: ".filtr-item", layoutMode: "fitRows" });
            $("ul.simplefilter li").on("click", function () {
                $("ul.simplefilter li").removeClass("active");
                $(this).addClass("active");
                var selector = $(this).attr("data-filter");
                $(".filtr-container").isotope({ filter: selector, animationOptions: { duration: 750, easing: "linear", queue: false } });
                return false;
            });
        });
        new WOW().init();
    ...
    $(document).ready(function() {
        ...
        $("#btnClose").click(function(e) {
            HideDialog(); // Assuming HideDialog() is defined elsewhere
            e.preventDefault();
        });
    });
    // $('.cdate').datepicker({
