...
class CommonRepository
{
    ...
    public function employeesList()
    {
        ...
        $options = ['' => '---- Please select ----'];
        foreach ($results as $key => $value) {
            $options[$value->emp_id] = $value->name . ' ( ' . $value->phone . ' - ' . $value->father_name . ' )';
        }
        return $options;
    }
    ...
    {
        $results = Company::where('status', 1)->get();
        $options = ['' => '---- Please select ----'];
        foreach ($results as $key => $value) {
            $options[$value->company_id] = $value->company_name;
        }
        return $options;
    }

    public function jobEmployeesList()
    {
        $results = AssignJob::select('assignjob.emp_id', 'employees.name', 'employees.phone', 'employees.father_name')
            ->join('employees', 'employees.emp_id', 'assignjob.emp_id')
            ->get();
        $options = ['' => '---- Please select ----'];
        foreach ($results as $key => $value) {
            $options[$value->emp_id] = $value->name . ' ( ' . $value->phone . ' - ' . $value->father_name . ' )';
